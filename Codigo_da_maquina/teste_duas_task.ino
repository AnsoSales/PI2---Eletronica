...
//  while(!digitalRead(B) && (micros() - previousMicros) <= timeout); // wait for the echo pin HIGH or timeout
//  previousMicros = micros();
//  while(digitalRead(B)  && (micros() - previousMicros) <= timeout); // wait for the echo pin LOW or timeout
//
//  tempo = micros() - previousMicros; // duration
}
void setup()
{
