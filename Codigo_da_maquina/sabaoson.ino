...
float tempo, volume1,volume2,volume3,volume4, dist1, dist2, media1=0, media2=0,media3=0,media4=0,h=0,dist,media = 0,leituras[numLeituras], altura, raio, Voltage1=0,Voltage2=0,PH,tempC=0.0, media7 = 0.0;
...
void nivel_essencia1(gpio_num_t A, gpio_num_t B)
{
  total = total - leituras[leituraAtual];
  sonico(A,B);
  dist= tempo*0.034/2;
...
void nivel_essencia2(gpio_num_t A, gpio_num_t B)
{
  total = total - leituras[leituraAtual];
  sonico(A,B);
  dist= tempo*0.034/2;
  ...
  media4 = total / numLeituras; 
  
    h=7.85-media4;

volume4=(h)*(Ainf+Asup+sqrt(Ainf*Asup))/3000;
}
...

{
   for(int i = 0; i<10; i++)
  {
    ...
    Voltage2 = (media/ 4095.0) * 3.3;
    PH = (Voltage2*(-5.7)) + 21.44; 
    media=0;
}
void setup()
{
...
void loop()//O loop() sempre será atribuído ao core 1 automaticamente pelo sistema, com prioridade 1
{
  ...
  //delay(60);
  //nivel_essencia1(trigger,echo2);
  //delay(60);
  //nivel_essencia2(trigger, echo1);
  //delay(60);
  char txString[8], txString2[8], txString3[8], txString4[8], txString5[8];//,txString6[8];
  
  dtostrf(volume1, 2, 2, txString);
  ...
  characteristicTX3->setValue(txString4);
  characteristicTX3->notify();
  dtostrf(tempC, 2, 2, txString5);
  characteristicTX4->setValue("temperatura"); //seta o valor que a caracteristica notificará (enviar)
